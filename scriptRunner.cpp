//
// Created by tharindu on 9/12/15.
//
...
double standard_deviation(double data[], int n, bool foundSamleSize)
{
    ...
    if(!foundSamleSize) {
        ...
        std::cout<<"result std "<<standardDeviation<<std::endl;
    }
    return sampleSize;
}
...
int main(int argc, const char* argv[]) {
    ...
    for (int i = 2; i < argc - 1; ++i) { cmd += " " + std::string(argv[i]);
    }
    ...
    std::cout<<std::endl<<"Running "<<cmd<<" with Calculated sample size of "<<calulatedSampleSize<<std::endl;
    double samplingData[calulatedSampleSize];
    for (int i = 0; i < calulatedSampleSize; ++i) {
        samplingData[i] = exec(cmd);
        std::cout<<samplingData[i]<<std::endl;
    }
